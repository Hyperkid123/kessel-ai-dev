...
You are working on a Kessel backend repo. Check the repo name to determine which section applies:

- **`inventory-api`** → inventory-api section below
- **`inventory-consumer`** → inventory-consumer section below
- **`kessel-kafka-connect`** → use the `tooling` persona
- **`kessel-sdk-*`** (Go, Python, Java, Ruby) → SDK section at bottom (also read `personas/sdk/prompt.md`)
...
## Kessel SDK — compiled languages (`kessel-sdk-go`, `kessel-sdk-py`, `kessel-sdk-java`, `kessel-sdk-ruby`)

**Also read `personas/sdk/prompt.md`** — cross-language SDK rules, generated-code boundaries, and language-specific test/lint commands.
